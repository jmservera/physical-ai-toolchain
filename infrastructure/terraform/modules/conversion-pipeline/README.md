...
Azure Blob immutability is container-scoped. Because `raw/` and `converted/` share the platform `datasets` container, the module applies one time-based policy to `datasets` using the larger of the raw and converted retention values. The separate `event-grid-dlq` container receives its own policy when dead-lettering is enabled.
...
| `should_enable_immutability_policy`          | `true`                          | Apply time-based immutability policies to `datasets` and `event-grid-dlq`                                                                              |
| `raw_immutability_period_in_days`            | `90`                            | Raw-data retention target. The effective `datasets` container policy uses the larger of the raw and converted values                                   |
| `converted_immutability_period_in_days`      | `30`                            | Converted-data retention target. The effective `datasets` container policy uses the larger of the raw and converted values                             |
| `event_grid_dlq_immutability_period_in_days` | `14`                            | Dead-letter retention for the `event-grid-dlq` container                                                                                               |
...
The module now enforces time-based immutability on the shared `datasets` container and, when enabled, on the separate `event-grid-dlq` container. The prior `CKV2_AZURE_50` deferral is resolved in code rather than carried as a README deferral.
...
| Container immutability | `should_enable_immutability_policy`    | Applies time-based WORM retention to `datasets` and `event-grid-dlq` |
